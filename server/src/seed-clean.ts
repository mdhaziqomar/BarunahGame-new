...
async function main() {
  ...
  console.log('✅ Clean data seeding completed!');
  console.log(`📚 ${realQuestions.length} questions added`);
  console.log(`🎁 ${realRewards.length} rewards created`);
  console.log('👤 Admin account: admin@barunah.edu.bn / admin123');
  console.log('🏪 Vendor account: vendor@local-store.bn / vendor123');
}

main()
  .catch((e) => {
    console.error('❌ Error during clean data seeding:', e);
    process.exit(1);
  })
  .finally(async () => {
